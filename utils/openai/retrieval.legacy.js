 ...
 * @param {boolean} _isAdapterCall - INTERNAL parameter to prevent recursive adapter loops
...
export async function identifyRelevantFiles(
  ...
  _isAdapterCall = false
) {
  // Call the adapter implementation with proper options mapping
  // But ONLY if this is not already an adapter call (prevents infinite recursion)
  if (
    !_isAdapterCall &&
    process.env.USE_REPOSITORY_PATTERN === "true" &&
    process.env.ENABLE_RETRIEVAL_ADAPTER === "true"
  ) {
    try {
  ...
  // Original implementation continues below for fallback or when adapters are disabled
  try {
 ...
 */
export async function retrieveDataFiles(fileIds, _isAdapterCall = false) {
  // Call the adapter implementation, but ONLY if this is not already an adapter call
  if (
    !_isAdapterCall &&
    process.env.USE_REPOSITORY_PATTERN === "true" &&
    process.env.ENABLE_RETRIEVAL_ADAPTER === "true"
