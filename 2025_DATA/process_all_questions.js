...
// Output file path
const outputFile = path.join(__dirname, "all_questions_mapped.csv");
...
// Function to process all CSV files and append results to a single output file
function processAllQuestions() {
  try {
    // Get all CSV files in the directory that match the pattern q*_global.csv
    const dataDir = __dirname;
    const files = fs
      .readdirSync(dataDir)
      .filter((file) => file.match(/^q\d+_global\.csv$/))
    ...
    // Process each file and collect all mapped rows
    let allMappedRows = [];

    for (const file of files) {
      const mappedRows = processCSVFile(file);
      allMappedRows = allMappedRows.concat(mappedRows);
    }

    console.log(`Total rows processed: ${allMappedRows.length}`);

    // Write all mapped rows to the output CSV
    ...
    console.log(`Mapping complete. All mapped data written to ${outputFile}`);
  } catch (error) {
