...
export class DataRetrievalService {
  ...
  filterDataBySegments(loadedData, segments) {
    ...
    try {
      // Use the adapter's processQueryWithData with appropriate context
      const context = {
        segments: segments,
        threadId: "data-retrieval-service",
        isFollowUp: false,
      };

      // Create a custom query context for filtering
      const result = processQueryWithData(
        "filter-only", // Empty query for filtering-only
        "all-sector",
        [], // No cached files
        context.threadId,
        context.isFollowUp,
